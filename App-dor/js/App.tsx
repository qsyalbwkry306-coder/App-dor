...
import { AppState, Order, Customer, Expense, InventoryItem } from './types';
...
const App: React.FC = () => {
  ...
  const triggerCloudSync = () => {
    setIsSyncing(true);
    // محاكاة عملية الرفع لسحابة جوجل
    setTimeout(() => {
  ...
  const handleGoogleLink = () => {
    setIsSyncing(true);
    // محاكاة عملية الربط بـ Google Auth
    setTimeout(() => {
              ...
              {isGoogleLinked ? (
                ...
                <div className="w-full space-y-3">
                  <button 
                    ...
                    disabled={isSyncing}
                    className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-bold shadow-lg shadow-indigo-100 flex items-center justify-center gap-3 active:scale-95 transition-all disabled:opacity-70"
                  >
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 shadow-lg px-2 py-1 z-40">
        <div className="flex justify-around items-center max-w-lg mx-auto h-16">
          {NAV_ITEMS.map((item) => (
            ...
            >
              <div className={`p-1 rounded-lg ${activeTab === item.id ? 'bg-indigo-50' : ''}`}>
                {item.icon}
              </div>
