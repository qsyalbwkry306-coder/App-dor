    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
          <div className="flex items-center gap-2 text-green-600 mb-2"><TrendingUp size={16} /><span className="text-xs font-bold">إجمالي المحصل</span></div>
          <div className="text-xl font-bold">{totalPaid.toLocaleString()} ريال</div>
        </div>
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
          <div className="flex items-center gap-2 text-red-600 mb-2"><TrendingDown size={16} /><span className="text-xs font-bold">إجمالي المصاريف</span></div>
      ...
      <div className="bg-indigo-600 p-5 rounded-2xl shadow-md text-white relative overflow-hidden">
        <div className="relative z-10">
          ...
          <div className="mt-4 flex items-center gap-2 text-xs"><span className="bg-white/20 px-2 py-1 rounded">المبالغ المستلمة - المصاريف</span></div>
        </div>
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
        <h3 className="text-sm font-bold mb-4 flex items-center gap-2"><Clock size={16} className="text-indigo-500" />طلبات قيد التصنيع</h3>
        <div className="space-y-3">
          {data.orders.filter(o => o.status === 'تحت التصنيع').map(order => (
            <div key={order.id} className="flex justify-between items-center p-3 bg-slate-50 rounded-xl">
              ...
              <div className="text-xs font-medium px-2 py-1 bg-blue-100 text-blue-700 rounded-lg">جاري العمل</div>
            </div>
          ))}
          {data.orders.filter(o => o.status === 'تحت التصنيع').length === 0 && (<p className="text-center text-slate-400 text-sm py-4">لا توجد طلبات جارية حالياً</p>)}
        </div>
      </div>
    </div>
  );
