    ...
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col items-center">
        ...
        <div className="grid grid-cols-2 w-full gap-4 mt-6 pt-6 border-t border-slate-50">
          <div className="text-center">
            <div className="text-green-500 flex items-center justify-center gap-1 text-xs font-bold mb-1">
              <ArrowUpCircle size={14} /> الإيرادات
            </div>
            <div className="text-lg font-bold">{totalRevenue.toLocaleString()}</div>
          </div>
          <div className="text-center border-r border-slate-50">
            <div className="text-red-500 flex items-center justify-center gap-1 text-xs font-bold mb-1">
              <ArrowDownCircle size={14} /> المصاريف
            </div>
      ...
      <div className="flex justify-between items-center px-1">
        ...
        <button 
          ...
          className="text-indigo-600 flex items-center gap-1 text-sm font-bold bg-indigo-50 px-3 py-1 rounded-full"
        >
          <Plus size={16} /> إضافة صرف
        </button>
      </div>
        ...
        {data.expenses.map(exp => (
          <div key={exp.id} className="bg-white p-4 rounded-2xl border border-slate-50 shadow-sm flex justify-between items-center">
            ...
            <div className="flex items-center gap-4">
              ...
              <button onClick={() => removeExpense(exp.id)} className="text-slate-300 hover:text-red-400">
                <Trash2 size={16} />
              </button>
            </div>
          </div>
          ...
          <div className="bg-white w-full max-w-sm rounded-[2rem] p-6 shadow-2xl">
            ...
            <form onSubmit={handleAddExpense} className="space-y-4">
              ...
              <div>
                ...
                <input required type="number" className="w-full p-3 bg-slate-50 rounded-xl" placeholder="0" onChange={e => setNewExpense({...newExpense, amount: +e.target.value})} />
              </div>
              <div>
                <label className="text-xs text-slate-400 block mb-1">الوصف</label>
                <input required type="text" className="w-full p-3 bg-slate-50 rounded-xl" placeholder="مثلاً: شراء مقابض حديد" onChange={e => setNewExpense({...newExpense, description: e.target.value})} />
              </div>
