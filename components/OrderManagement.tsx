...
import { Plus, Search, Filter, Phone, ChevronRight, Edit2, ArrowUpDown } from 'lucide-react';
...
const OrderManagement: React.FC<Props> = ({ data, updateData }) => {
  ...
  const handleSaveOrder = (e: React.FormEvent) => {
    ...
    if (editingOrder) {
      // Update existing order and customer
      const updatedOrders = data.orders.map(o => 
        o.id === editingOrder.id ? { ...o, ...formData } as Order : o
      );
      const updatedCustomers = data.customers.map(c => 
        c.id === editingOrder.customerId ? { ...c, name: customerName, phone: customerPhone } : c
      );
      updateData({ orders: updatedOrders, customers: updatedCustomers });
    } else {
      // Add new order and customer
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full pr-10 pl-4 py-2.5 bg-white border border-slate-200 rounded-2xl focus:ring-2 focus:ring-indigo-500 outline-none shadow-sm"
              value={searchTerm}
          ...
          <button 
            ...
            className="bg-indigo-600 text-white p-3 rounded-2xl shadow-lg shadow-indigo-200 hover:bg-indigo-700 active:scale-95 transition-all"
          >
            <Plus size={24} />
          </button>
        </div>
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-hide">
          <div className="flex items-center gap-1 text-xs font-bold text-slate-400 shrink-0 ml-2">
            <ArrowUpDown size={14} /> فرز حسب:
          </div>
          <button 
            onClick={() => setSortBy('date-new')}
            className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-colors border ${sortBy === 'date-new' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-500 border-slate-200'}`}
          >
            الأحدث
          </button>
          <button 
            onClick={() => setSortBy('price-high')}
            className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-colors border ${sortBy === 'price-high' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-500 border-slate-200'}`}
          >
            الأعلى سعراً
          </button>
          <button 
            onClick={() => setSortBy('price-low')}
            className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-colors border ${sortBy === 'price-low' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-500 border-slate-200'}`}
          >
            الأقل سعراً
          </button>
        </div>
            ...
            <div key={order.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-col gap-3 group relative overflow-hidden">
              ...
              <div className="flex items-center gap-4 py-2 border-y border-slate-50">
                <div className="flex-1">
                  <p className="text-[10px] text-slate-400 uppercase font-bold">النوع</p>
                  <p className="text-sm font-semibold">{order.doorType}</p>
                </div>
                <div className="flex-1">
                  <p className="text-[10px] text-slate-400 uppercase font-bold">المقاس</p>
                  <p className="text-sm font-semibold">{order.dimensions.width}×{order.dimensions.height}</p>
                </div>
                <div className="flex-1 text-left">
                  <p className="text-[10px] text-slate-400 uppercase font-bold">السعر</p>
                  <p className="text-sm font-bold text-indigo-600">{order.totalPrice} ر.س</p>
                </div>
              </div>

              {order.notes && (
                <p className="text-[11px] text-slate-500 italic bg-slate-50 p-2 rounded-lg border-r-2 border-indigo-300 line-clamp-2">
                  {order.notes}
                </p>
              )}
              ...
              <div className="flex justify-between items-center text-xs">
                ...
                <div className="flex gap-2">
                  ...
                  <button className="text-indigo-600 font-bold flex items-center gap-1 px-2">
          ...
          <div className="bg-white w-full max-w-lg rounded-t-[2.5rem] p-6 animate-in slide-in-from-bottom-full duration-300 shadow-2xl">
            ...
            <form onSubmit={handleSaveOrder} className="space-y-4 max-h-[70vh] overflow-y-auto px-1 custom-scrollbar pb-6">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <select className="flex-1 px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl outline-none" value={formData.status} onChange={e => setFormData({...formData, status: e.target.value as any})}>
                    ...
                    <option value="تحت التصنيع">تحت التصنيع</option>
                ...
                <div className="flex gap-2">
                  <div className="w-1/2">
                    <label className="text-[10px] text-slate-400 mr-2">العرض (سم)</label>
                    <input required placeholder="العرض" type="number" className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl" value={formData.dimensions?.width} onChange={e => setFormData({...formData, dimensions: {...formData.dimensions!, width: +e.target.value}})} />
                  </div>
                  <div className="w-1/2">
                    <label className="text-[10px] text-slate-400 mr-2">الارتفاع (سم)</label>
                    <input required placeholder="الارتفاع" type="number" className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl" value={formData.dimensions?.height} onChange={e => setFormData({...formData, dimensions: {...formData.dimensions!, height: +e.target.value}})} />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <div className="w-1/2">
                    <label className="text-[10px] text-slate-400 mr-2">السعر الإجمالي</label>
                    <input required placeholder="الإجمالي" type="number" className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl" value={formData.totalPrice} onChange={e => setFormData({...formData, totalPrice: +e.target.value})} />
                  </div>
                  <div className="w-1/2">
                    <label className="text-[10px] text-slate-400 mr-2">المبلغ المدفوع</label>
                    <input required placeholder="المدفوع" type="number" className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl" value={formData.paidAmount} onChange={e => setFormData({...formData, paidAmount: +e.target.value})} />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <textarea 
                  placeholder="أدخل أي ملاحظات خاصة بالطلب هنا..."
                  className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl outline-none focus:border-indigo-500 transition-colors min-h-[80px] text-sm"
                  value={formData.notes}
                  onChange={e => setFormData({...formData, notes: e.target.value})}
                />
              </div>

              <button type="submit" className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-bold shadow-lg shadow-indigo-100 active:scale-[0.98] transition-all">
                {editingOrder ? 'تحديث البيانات' : 'حفظ الطلب الجديد'}
              </button>
            </form>
          </div>
        </div>
