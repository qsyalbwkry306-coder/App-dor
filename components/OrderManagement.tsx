...
const OrderManagement: React.FC<Props> = ({ data, updateData }) => {
  ...
  const resetForm = () => {
    setFormData({ doorType: 'حديد', status: 'قيد الانتظار', dimensions: { width: 0, height: 0 }, totalPrice: 0, paidAmount: 0, notes: '' });
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => { resetForm(); setShowForm(true); }} className="bg-indigo-600 text-white p-3 rounded-2xl shadow-lg shadow-indigo-200 hover:bg-indigo-700 active:scale-95 transition-all"><Plus size={24} /></button>
        </div>
          ...
          return (
            <div key={order.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex flex-col gap-3">
              ...
              <div className="flex items-center gap-4 py-2 border-y border-slate-50">
                ...
                <div className="flex-1"><p className="text-[10px] text-slate-400 uppercase font-bold">المقاس</p><p className="text-sm font-semibold">{order.dimensions.width}×{order.dimensions.height} سم</p></div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-end justify-center">
          <div className="bg-white w-full max-w-lg rounded-t-[2.5rem] p-6 shadow-2xl animate-in slide-in-from-bottom-full duration-300">
            <div className="flex justify-between items-center mb-6"><h3 className="text-lg font-bold">{editingOrder ? 'تعديل الطلب' : 'طلب جديد'}</h3><button onClick={() => setShowForm(false)} className="text-slate-400 p-2">إغلاق</button></div>
