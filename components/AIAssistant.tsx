    ...
    <div className="flex flex-col h-[calc(100vh-12rem)]">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-4 px-1 custom-scrollbar pb-4">
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] p-3 rounded-2xl flex gap-2 ${m.role === 'user' ? 'bg-white border border-slate-100 rounded-br-sm text-slate-700' : 'bg-indigo-50 text-indigo-900 rounded-bl-sm'}`}>
      ...
      <div className="mt-auto space-y-4">
        ...
        <div className="relative">
          <input type="text" placeholder="اسألني عن مبيعاتك أو مصاريفك..." className="w-full pl-4 pr-14 py-4 bg-white border border-slate-200 rounded-2xl shadow-sm outline-none focus:ring-2 focus:ring-indigo-500" value={input} onChange={e => setInput(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleSend()} />
          <button onClick={handleSend} disabled={loading} className="absolute right-2 top-2 bottom-2 w-10 bg-indigo-600 text-white rounded-xl flex items-center justify-center active:scale-95 transition-all disabled:bg-slate-300"><Send size={18} /></button>
        </div>
      </div>
    </div>
  );
